...
layui.define(['jquery', 'layer', 'form'], function (exports) {
    ...
    var LI_NAV_CHILD = "dtree-nav-ul-sid", LI_NAV_ITEM = "dtree-nav-item",
        ...
        LI_DIV_MENUBAR = "dtree-menubar", LI_DIV_MENUBAR_DOWN = "dtree-icon-move-down",
        LI_DIV_MENUBAR_UP = "dtree-icon-move-up", LI_DIV_MENUBAR_REFRESH = "dtree-icon-refresh",
        LI_DIV_MENUBAR_DELETE = "dtree-icon-delete1", LI_DIV_MENUBAR_SEARCH = "dtree-icon-search_list_light",
        LI_DIV_TOOLBAR = "dtree-toolbar", TOOLBAR_TOOL = "dtree-toolbar-tool",
        LI_DIV_TOOLBAR_ADD = "dtree-icon-roundadd", LI_DIV_TOOLBAR_EDIT = "dtree-icon-bianji",
        LI_DIV_TOOLBAR_DEL = "dtree-icon-roundclose",
        LI_DIV_SPREAD_LAST = "dtree-icon-dian",
        LI_DIV_CHECKBAR = "dtree-nav-checkbox-div", LI_DIV_CHECKBAR_ON = "dtree-icon-fuxuankuangxuanzhong",
        LI_DIV_CHECKBAR_OUT = "dtree-icon-fuxuankuang", LI_DIV_CHECKBAR_NOALL = "dtree-icon-fuxuankuang-banxuan",
...
//			level: "level",			//层级
    ...
    DTree.prototype.setting = function (options) {
        ...
        this.success = this.options.success || function (data, obj) {
        };		//树加载完毕后执行解析树之前的回调（仅限异步加载）
        this.done = this.options.done || function (data, obj) {
        };		//树加载完毕后的回调（仅限异步加载）
    ...
    DTree.prototype.changeTreeNodeDel = function (flag) {
        ...
        if (flag) {
            ...
            if ($p_ul.children("li").length == 0) {
                ...
                $icon_i.eq(0).removeClass(_this.ficonClose);
                if (!_this.dot) {
                    $icon_i.eq(0).addClass(ICON_HIDE);
                }
